...
15. Windows 程序的窗口函数是一个（B）。  
  A. 回调函数  
  **B. API 函数**  
  C. 主函数  
  D. 绘图函数
    > [Windows 程序的窗口函数是一个（B）。 | 单项选择题](/notes/one-choice/#Windows-程序的窗口函数是一个（B）。)
...
19. 正在开发一个没有人真正需要的软件产品,这是遭遇了（C）风险。  
  A. 项目  
  B. 技术  
  **C. 商业**  
  D. 其它
...
1. 人们曾采用的软件工程方法学有：结构化技术、（**面向对象技术**）、面向组件技术。
2. 快速原型法最根本的任务就是为获取用户真实的（**需求**）。
3. 面向对象软件是由一系列对象组成，对象之间通过（**消息**）互相协作。
4. 面向对象需求分析可以再细分为两个步骤：（**需求陈述**）和分析建模。
5. 面向对象需求分析的任务是：抽取和整理用户的需求，建立（**问题域**）的精确模型。
6. 子系统之间的关系有两种：“客户机/服务器”关系和（**对等关系**）。
7. 主要存在三种软件的控制范式：过程驱动型、（**对象型**）和并发型。
8. 动态模型的（**事件跟踪**）和功能模型的“数据处理”常对应着类中的服务。
9. 软件调试就是把症状和（**原因**）联系起来的智力过程。
10. 为了估计软件项目的工作量和完成期限,首先要度量软件的（**规模**）。
...
4. 在面向对象的问题域模型中，动态模型是最基本和最重要的。（❌）
   > 对象模型是最基本和最重要的。
5. 面向对象开发方法就是把程序分解成一组互相协作的对象。（✔️）
6. 一个软件的子系统可以通过它所提供的服务来标识。（✔️）
7. 在客户机/服务器关系中,服务器需要知道客户机的接口。（❌）
   > 应该反过来。
8. 详细设计主要是一个优化和加入实现对象的过程。（❌）
   > 详细设计：把解法具体化，详细地设计每个模块，确定实现模块功能所需要的算法和数据结构。
9. 软件测试的目标是为了使程序没有错误。（❌）
    > 测试的目的就是在软件投入生产性运行之前， 尽可能多地发现软件中的错误。
10. 动态多变量模型是以软件规模和开发时间为自变量的函数。（✔️）

## 四、名词解释（每小题 4 分，共 20 分）

1. [软件生命周期](/notes/explanation-of-nouns/#软件生命周期)
2. [耦合](/notes/explanation-of-nouns/#耦合)
3. [面向对象分析](/notes/explanation-of-nouns/#面向对象分析)
4. [软件调试](/notes/explanation-of-nouns/#软件调试)
5. [软件质量](/notes/explanation-of-nouns/#软件质量)

## 五、简答题（每小题 5 分，共 20 分）

1. [简述软件工程的目标。](/notes/short-answer/#简述软件工程的目标。)
2. [简述三种面向对象模型的主要功能。](/notes/short-answer/#简述三种面向对象模型的主要功能。)
3. [简述软件开发中文档的重要性。](/notes/short-answer/#简述软件开发中文档的重要性。)
4. [简述软件设计的“信息隐藏原则”。](/notes/short-answer/#简述软件设计的“信息隐藏原则”。)
